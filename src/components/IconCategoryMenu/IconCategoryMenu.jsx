...
export const IconCategoryMenu = ({ images, categoryImgId, setCategoryImgId }) => {
  const [openIconDrop, setOpenIconDrop] = useState(false);

  return (
    <>
      { images.error ? (
        <ErrorFetch summaryStyle={styles.iconCategorySummary} errorName={images.name} />
      ) : (
        <details className={styles.iconCategoryDetails} open={openIconDrop} onClick={(e) => {e.preventDefault();setOpenIconDrop(!openIconDrop);}}>
          <summary className={styles.iconCategorySummary}>
            <span>{ !categoryImgId ? "Выберите иконку" : images[categoryImgId - 1].img_name}</span>
            ...
            <div className={styles.iconCategoryDrop}>
              <div className={styles.iconCategoryScroll}>
                <div className={styles.iconCategoryOptions}>
                  { images.map((el, idx) => (
                    <div className={styles.iconCategoryIcon} onClick={() => {setCategoryImgId(el.id);setOpenIconDrop(!openIconDrop)}} key={idx}>
                      <img src={`/images/icons/${el.img_name}`} alt="icon" />
                    </div>
                  )) }
                </div>
              </div>
