...
import { deletePayment } from "../../store/payments/actions";
import { apiPayments } from "../../utils/constants";
import { hideEdit, showEdit, showModal } from "../../store/modal/actions";
...
export const PaymentList = ({ paymentList }) => {
  ...
  const openEditMenu = (e, payment) => {
    e.stopPropagation();
    dispatch(showEdit(payment));
  };
  ...
  const handleDeletePayment = (sum) => {
    dispatch(deletePayment(`${apiPayments}/${showEditId}`, "DELETE", {balance: sum}));
    dispatch(hideEdit());
  };
        ...
        <div className={styles.expItem} key={idx}>
          <div className={styles.expLeft}>
            <div className={styles.expIcon}>
              <img src={`/images/icons/${payment.categoryImg}`} alt="icon" />
            </div>
          ...
          <div className={styles.expRight}>
            <span className={styles.expSum}>- {payment.sum.toLocaleString()} &#8381;</span>
            <button type="button" className={styles.expBtnEdit} onClick={(e) => openEditMenu(e, payment)}><img src="/images/Edit.png" alt="Edit" /></button>
            { payment.id === showEditId &&
              <div className={styles.expShowEditDelete}>
                ...
                <button type="button" onClick={() => handleDeletePayment(payment.sum)}>
