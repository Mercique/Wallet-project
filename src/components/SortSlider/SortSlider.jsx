...
import styles from "./SortSlider.module.css";

export const SortSlider = ({ categoryList, categoryEdit, handleActive }) => {
  const [top, setTop] = useState(0);
  const [step, setStep] = useState(0);

  const handleUp = () => {
    if (step !== 0) {
      setTop(prevTop => prevTop + 122);
  ...
  return (
    <div className={styles.sortSliderWrapper}>
      { categoryList?.length > 5 &&
        <button type="button" className={styles.sortSliderArrow} onClick={handleUp} disabled={step === 0}>
      ...
      <div className={styles.categoryIconWrapper}>
        <div id="sliderLine" className={styles.categoryIconLine} style={{ top: `${top}px` }}>
      ...
      { categoryList?.length > 5 &&
        <button type="button" className={styles.sortSliderArrow} onClick={handlebottom} disabled={step + 5 === categoryList?.length}>
          <svg width="50" height="30" viewBox="0 0 50 30" fill="none" xmlns="http://www.w3.org/2000/svg">
